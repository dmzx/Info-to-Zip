...
class infotozip_preview_button
{
	/** @var auth */
	protected $auth;
	...
	*
	* @param auth							$auth
	* @param template\template				$template
	* @param user							$user
	...
	*/
	...
	function main($urlcodec)
	{
		...
		$enl_desc		= "{$this->root_path}infotozip?mode=download&urlcodec=" . $cat_url;

		// Check enabled.
		if ($this->config['infotozip_enable'])
		{
			$download = $enl_desc;
			$link_colour = 'light-blue';
			$link_redirect = '';
			$link_no_arrow = '';
			$title = $this->user->lang['INFOTOZIP_LINK_ADD_FREE_FILE_DOWN'];
			$text = $this->user->lang['INFOTOZIP_LINK_ADD_FREE_DOWN'];
		}
		else
		{
			$download = '';
